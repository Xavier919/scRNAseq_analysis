...
class SplineLinearLayer(nn.Module):
    def __init__(self, input_dim, output_dim, num_knots=5, spline_order=3,
        ...
        self.base_weights = nn.Parameter(torch.Tensor(output_dim, input_dim))
        self.spline_weights = nn.Parameter(torch.Tensor(output_dim, input_dim, num_knots + spline_order))
        if standalone_spline_scaling:
            self.spline_scales = nn.Parameter(torch.Tensor(output_dim, input_dim))
    ...
    def _initialize_parameters(self):
        nn.init.xavier_uniform_(self.base_weights, gain=torch.sqrt(torch.tensor(2.0)))
        ...
        if self.standalone_spline_scaling:
            nn.init.xavier_uniform_(self.spline_scales, gain=torch.sqrt(torch.tensor(2.0)))
...
class DeepKAN(nn.Module):
    ...
    """
    def __init__(self, input_dim, shared_layers, dual_layers, num_knots=5, spline_order=3,
                 noise_scale=0.1, base_scale=1.0, spline_scale=1.0,
                 activation=nn.SiLU, grid_epsilon=0.02, grid_range=[-1, 1]):
        super(DeepKAN, self).__init__()

        self.shared_layers = nn.ModuleList()
        layers = [input_dim] + shared_layers
        for in_dim, out_dim in zip(layers, layers[1:]):
            self.shared_layers.append(SplineLinearLayer(in_dim, out_dim, num_knots, spline_order,
                                                        noise_scale, base_scale, spline_scale,
                                                        activation, grid_epsilon, grid_range))

        self.dual1 = nn.ModuleList()
        layers = [shared_layers[-1]] + dual_layers
        for in_dim, out_dim in zip(layers, layers[1:]):
            self.dual1.append(SplineLinearLayer(in_dim, out_dim, num_knots, spline_order,
                                                noise_scale, base_scale, spline_scale,
                                                activation, grid_epsilon, grid_range))

        self.dual2 = nn.ModuleList()
        layers = [shared_layers[-1]] + dual_layers
        for in_dim, out_dim in zip(layers, layers[1:]):
            self.dual2.append(SplineLinearLayer(in_dim, out_dim, num_knots, spline_order,
                                                noise_scale, base_scale, spline_scale,
                                                activation, grid_epsilon, grid_range))
    ...
    def forward(self, x, update_knots=False):
        ...
        for layer in self.dual1:
            if update_knots:
                layer._update_knots(x1)
            x1 = layer(x1)

        for layer in self.dual2:
            if update_knots:
                layer._update_knots(x2)
            x2 = layer(x2)
    ...
    def regularization_loss(self, regularize_activation=1.0, regularize_entropy=1.0):
        ...
        """
        return sum(layer._regularization_loss(regularize_activation, regularize_entropy) for layer in self.shared_layers + self.dual1 + self.dual2)
    

class SiameseKAN(nn.Module):
...
class DeepKAN(torch.nn.Module):
    def __init__(self, input_dim, hidden_layers, num_knots=3, spline_order=2,
                 noise_scale=0.1, base_scale=1.0, spline_scale=1.0,
                 activation=torch.nn.SiLU, grid_epsilon=0.02, grid_range=[-1, 1]):
        super(DeepKAN, self).__init__()
        layers = [input_dim] + hidden_layers
